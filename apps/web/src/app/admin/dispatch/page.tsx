...
// --- UPDATED: Using your final, complete admin menu items array ---
...
export default function DispatchOrders() {
  ...
  const [dispatchingId, setDispatchingId] = useState(null); // Tracks which order is being dispatched
  ...
  const handleDispatch = async (orderId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorResult.detail || 'Failed to create shipment.');
      }

      const shipmentResult = await response.json();
      alert(`Shipment created successfully! Shipment ID: ${shipmentResult.shipment_id}`);

      // Remove the dispatched order from the list for immediate UI feedback
      setOrders(prevOrders => prevOrders.filter(order => order.order_id !== orderId));

    } catch (err) {
      setError(err.message);
    } finally {
      setDispatchingId(null); // Stop the loading spinner for this button
    }
  };
  ...
  const renderContent = () => {
    ...
    if (orders.length === 0) return <p className="text-center text-gray-500">No pending orders to dispatch.</p>;

    return (
      <div className="space-y-4">
        {orders.map(order => (
          <div key={order.order_id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex justify-between items-center">
            <div>
              <p className="font-bold text-lg">{order.destination.city}</p>
              <p className="text-sm text-gray-500">Order ID: {order.order_id}</p>
              <p className="text-sm">Total: <span className="font-semibold">${order.order_total.toFixed(2)}</span></p>
            </div>
            <button
  ...
  return (
    <DashboardLayout role="admin" menuItems={adminMenuItems}>
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">Dispatch Pending Orders</h1>
        {renderContent()}
      </div>
    </DashboardLayout>
  );
