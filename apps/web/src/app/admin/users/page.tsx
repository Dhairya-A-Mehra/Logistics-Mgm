...
// Your array of menu items for the sidebar
...
export default function Users() {
  ...
  // This function fetches the list of all users from the backend
  ...
  // This hook calls fetchUsers() when the page first loads
  useEffect(() => {
    fetchUsers();
  }, []);
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      setUsers(prevUsers => prevUsers.filter(user => user.customer_id !== userId));
  ...
  // This function is called from the modal to refresh the user list
  const handleUserAdded = () => {
    fetchUsers(); 
  };
  const handleDataUpdate = () => {
    fetchUsers(); // A single function to refresh data after add or edit
  };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {users.map((user) => (
                    <tr key={user.customer_id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button 
                          ...
                          className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 mr-3 font-medium"
                        >
                          Edit
                        </button>
                        <button 
