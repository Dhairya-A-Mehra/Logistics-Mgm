    ...
    const fetchDashboardStats = async () => {
      try {
        ...
        setStats({
          ...
          deliverySuccessRate: deliveryMetrics?.[0]?.success_rate || 0,
          avgDeliveryTime: deliveryMetrics?.[0]?.avg_delivery_time || 0,
          customerSatisfaction: deliveryMetrics?.[0]?.avg_satisfaction || 0,
        });
      } catch (error) {
      ...
      <div className={`bg-gradient-to-br ${colors[color]} p-6 rounded-xl shadow-md text-white`}>
        ...
        {trend && (
          <p className="text-sm mt-2 opacity-75">
            {trendIcons[trend]} {trend === 'up' ? 'Increase' : trend === 'down' ? 'Decrease' : 'No change'} from last period
          </p>
        )}
      </div>
    );
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            {realtimeEnabled ? 'Live updates enabled' : 'Connecting...'}
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-gray-200 dark:bg-gray-700 p-6 rounded-xl shadow-md animate-pulse h-32" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <StatCard
              title="Revenue"
              value={`$${(stats.revenue || 0).toLocaleString(undefined, {
