...
const warehouses = [
  { id: 'WH-001', name: 'Delhi Central', location: 'Delhi', capacity: '10,000 sq ft', utilization: 75, status: 'active' },
  { id: 'WH-002', name: 'Mumbai Hub', location: 'Mumbai', capacity: '15,000 sq ft', utilization: 82, status: 'active' },
  { id: 'WH-003', name: 'Bangalore South', location: 'Bangalore', capacity: '8,000 sq ft', utilization: 45, status: 'active' },
  { id: 'WH-004', name: 'Chennai Port', location: 'Chennai', capacity: '12,000 sq ft', utilization: 90, status: 'maintenance' },
];
    ...
    <DashboardLayout role="admin" menuItems={adminMenuItems}>
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-colors">+ Add Warehouse</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">45K sq ft</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            <p className="text-sm text-gray-600 dark:text-gray-400">Avg Utilization</p>
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">73%</p>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Utilization</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {warehouses.map((warehouse) => (
                <tr key={warehouse.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-white">{warehouse.id}</td>
                  <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900 dark:text-white">{warehouse.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-400">{warehouse.location}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-400">{warehouse.capacity}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2 w-20">
                        <div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${warehouse.utilization}%` }}></div>
                      </div>
                      <span className="text-sm text-gray-600 dark:text-gray-400">{warehouse.utilization}%</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full ${warehouse.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'}`}>
                      {warehouse.status}
                    </span>
                  </td>
