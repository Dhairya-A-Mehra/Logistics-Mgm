...
export default function Analysis() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Orders revenue (sum by day for last 7 days)
        ...
        for (const o of orders || []) {
          ...
          const amt = typeof o.amount === 'number' ? o.amount : parseFloat(o.amount as any);
          if (!Number.isNaN(amt) && key in byDay) byDay[key] += amt;
        }
        const revPoints: RevenuePoint[] = Object.keys(byDay)
          .sort()
          .map((k, idx) => ({ label: last7Labels[idx], value: Math.round(byDay[k]) }));
        setRevenueTrend(revPoints);

        // Shipments status distribution (last 7 days)
  ...
  const totalStatuses = Object.values(statusDist).reduce((a, b) => a + b, 0) || 1;
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 p-6 rounded-xl shadow-md text-white">
            ...
            <p className="text-sm mt-2 opacity-75">↑ 2.1% from last month</p>
          </div>
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl shadow-md text-white">
            ...
            <p className="text-3xl font-bold mt-2">2.4 days</p>
            <p className="text-sm mt-2 opacity-75">↓ 0.3 days improved</p>
          </div>
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-xl shadow-md text-white">
            ...
            <p className="text-sm mt-2 opacity-75">↑ 0.2 from last month</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            ...
            {loading ? (
              <p className="text-sm text-gray-500 dark:text-gray-400">Loading revenue…</p>
            ) : (
              <>
                <div className="h-64 flex items-end justify-around gap-2">
                  {revenueTrend.map((pt, i) => {
                    const max = Math.max(1, ...revenueTrend.map(p => p.value));
                    const height = Math.round((pt.value / max) * 100);
                    return (
                      <div key={i} className="flex-1 bg-indigo-500 rounded-t" style={{ height: `${height}%` }} title={`$${pt.value}`}></div>
                    );
                  })}
                </div>
          ...
          {/* Delivery Status */}
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Delivery Status Distribution</h3>
            {loading ? (
              <p className="text-sm text-gray-500 dark:text-gray-400">Loading statuses…</p>
            ) : (
              <div className="space-y-4">
                {Object.keys(statusDist).length === 0 && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">No shipment data in the last 7 days.</p>
                )}
                  ...
                  return (
                    <div key={label}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-600 dark:text-gray-400 capitalize">{label.replace('_', ' ')}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Top Delivery Personnel</h3>
            <div className="space-y-3">
              {[
                { name: 'John Doe', deliveries: 145, rating: 4.9 },
                { name: 'Jane Smith', deliveries: 132, rating: 4.8 },
                { name: 'Bob Johnson', deliveries: 128, rating: 4.7 },
              ].map((person, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{person.name}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{person.deliveries} deliveries</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <svg className="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{person.rating}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            ...
            <div className="space-y-3">
              {[
                { route: 'Delhi → Mumbai', count: 234 },
                { route: 'Bangalore → Chennai', count: 189 },
                { route: 'Kolkata → Hyderabad', count: 156 },
              ].map((route, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{route.route}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{route.count} shipments</p>
                  </div>
